...
- (void)showNonetWork
{
    ...
    FDNotInternetView  * view = [[FDNotInternetView alloc] initWithFrame:CGRectMake(0, YY, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-YY)];
    view.delegate = self;
...
{
    if ([self respondsToSelector:@selector(loadDatas)]) {
...
- (void)loadDatas
{
    NSLog(@"必须由网络控制器(%@)重写这个方法(%@)，才可以使用再次刷新网络",NSStringFromClass([self class]),NSStringFromSelector(@selector(loadDatas)));
}
